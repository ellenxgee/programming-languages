...
(define (eval-statement env statement [slow? #f])
  ...
  (match statement 
    ...
    [(line-expr x1 y1 x2 y2)
     (draw-line (eval-expr env x1) (eval-expr env y1) (eval-expr env x2) (eval-expr env y2)) env]
    ...
    [(assignment-expr (var-expr lval) expr)
     (let ([lookup (apply-env env lval)]
	   [value (eval-expr env expr)])
       (if lookup
	   (begin (setref! lookup value) env)
           (extend-env env lval value)))]
    ...
    [(apply-expr sym exprs)
     ...
     (let ([args (map (lambda (element) (eval-expr env element)) exprs)]
	   ...
	   (begin
	     ...
	     (eval-statements (append env paramList) (closure-body func))) env)
